...
class DesktopProtocol:
            ...
            if stream:
                ...
                if '__interrupt__' in response:
                    ...
                    if interrupts:
                        ...
                        print(f"[_handle_chat] Interrupt detected (non-streaming): tool={tool_info['tool_name']}, args={tool_info['tool_input']}, id={tool_info['tool_call_id']}", file=sys.stderr)
                        
                        # Store original request ID for non-streaming too
                        self._original_chat_request_id = request_id
    ...
    async def _handle_tool_approval(self, request_id: str, params: dict) -> dict[str, Any]:
        ...
        for interrupt_id in self.pending_interrupts.keys():
            # Get original request to check for multiple tool calls
            original_request = self.pending_interrupts[interrupt_id]
            action_requests = original_request.get("action_requests", [])
            request_count = len(action_requests)
            
            print(f"[_handle_tool_approval] Interrupt {interrupt_id} has {request_count} action requests", file=sys.stderr)
            
            decisions = []
            for i in range(request_count):
                # For the first request, apply the user's decision
                if i == 0:
                    ...
                    if action == "edit" and tool_input:
                        ...
                        tool_name = "unknown_tool"
                        if action_requests:
                            tool_name = action_requests[0].get("name", "unknown_tool")
                    ...
                    # - If user rejected the first one, we reject all (cancel batch)
                    # - If user approved or edited the first one, we approve the rest (let them run as is)
                    if action == "reject":
            ...
            if isinstance(response, dict) and '__interrupt__' in response:
                ...
                if interrupts:
                    ...
                    if not original_request_id:
                         print(f"[_handle_tool_approval] WARNING: No original_request_id found for resume interrupt!", file=sys.stderr)
                         # Fallback to current request ID, though it might be wrong
    ...
    async def _handle_scan_github_for_skills(self, request_id: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            from deepagents_cli.skills.load import _parse_skill_metadata
            ...
            for root, dirs, files in os.walk(temp_dir):
                ...
                if 'SKILL.md' in files:
                    ...
                    skill_info = {
                        ...
                        'relative_path': str(skill_dir.relative_to(temp_path)),
                        'name': skill_dir.name,
                        'description': ''
                    }
                    ...
                    try:
                        metadata = _parse_skill_metadata(skill_md_path, source='github_scan')
                        if metadata:
                            skill_info['name'] = metadata['name']
                            skill_info['description'] = metadata['description']
                    except Exception as e:
                        print(f"[_handle_scan_github_for_skills] Failed to parse metadata for {skill_dir.name}: {e}", file=sys.stderr)
                    
                    found_skills.append(skill_info)
            
            # Sort skills by name
            found_skills.sort(key=lambda x: x['name'])

            # Do NOT delete temp_dir here - it is needed for import!
            # Cleanup happens in _handle_import_selected_skills
    ...
    async def _handle_import_selected_skills(self, request_id: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            for skill_info in selected_skills:
                # Use skill name from metadata if available, otherwise dir_name
                # This ensures directory name matches skill name per spec
                skill_name = skill_info.get('name', skill_info['dir_name'])
