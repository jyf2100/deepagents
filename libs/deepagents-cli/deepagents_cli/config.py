...
# 只从 ~/.deepagents/.env 读写配置，忽略当前目录的 .env
...
def _configure_proxy_bypass() -> None:
    """Automatically add private IPs from OPENAI_BASE_URL to NO_PROXY."""
    ...
    try:
        ...
        try:
            ...
            # Not an IP address (e.g. localhost or domain)
        ...
        if is_private:
            ...
            if no_proxy:
                if host not in no_proxy:
                    os.environ["NO_PROXY"] = f"{no_proxy},{host}"
            else:
                os.environ["NO_PROXY"] = host
            
            # Also set lowercase no_proxy for compatibility
...
class Settings:
    ...
    # === Workspace-level methods (organize data by agent/workspace) ===
    ...
    def ensure_workspace_skills_dir(self, agent_name: str, workspace_id: str) -> Path:
...
def _detect_provider(model_name: str) -> str | None:
    ...
    if any(x in model_lower for x in ["gpt", "o1", "o3", "qwen", "deepseek"]):
        return "openai"
...
def create_model(model_name_override: str | None = None) -> BaseChatModel:
    ...
    # Use environment variable defaults, detect provider by API key priority
        ...
        # Debug logging to diagnose model selection
        import sys
        print(f"[create_model] has_openai: {settings.has_openai}", file=sys.stderr, flush=True)
        print(f"[create_model] has_anthropic: {settings.has_anthropic}", file=sys.stderr, flush=True)
        print(f"[create_model] has_google: {settings.has_google}", file=sys.stderr, flush=True)
        print(f"[create_model] OPENAI_MODEL env: {os.environ.get('OPENAI_MODEL', 'NOT SET')}", file=sys.stderr, flush=True)
        print(f"[create_model] ANTHROPIC_MODEL env: {os.environ.get('ANTHROPIC_MODEL', 'NOT SET')}", file=sys.stderr, flush=True)
