...
class SkillsMiddleware(AgentMiddleware):
    ...
    Args:
        ...
        enabled_skills: Optional list of enabled skill names for filtering.
                         If None or contains "*", all skills are enabled.
    """
    ...
    def _filter_skills(self, skills: list[SkillMetadata]) -> list[SkillMetadata]:
        """Filter skills based on enabled_skills configuration.
        ...
        """
        if self.enabled_skills is None or "*" in self.enabled_skills:
            return skills

        # Filter to only enabled skills (match by dir_name)
        enabled_set = set(self.enabled_skills)
        return [s for s in skills if s["dir_name"] in enabled_set]
    ...
    def before_agent(self, state: SkillsState, runtime: Runtime) -> SkillsStateUpdate | None:
        ...
        # Apply workspace-specific filtering if configured
